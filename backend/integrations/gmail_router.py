...
@router.get("/login")
async def gmail_login(db: AsyncSession = Depends(get_db)):
...
async def gmail_auth_alias(db: AsyncSession = Depends(get_db)):
    """Alias for /login to maintain backward compatibility if needed."""
    return await gmail_login(db)
